...
lengthtopredict <- ((max(alligator$lnLength)-min(alligator$lnLength))/100 * perc)+min(alligator$lnLength)

predictedweight <- predict(alli.mod1, lengthtopredict)

alligatorprediction <- data.frame(lnLength = lengthtopredict, lnWeight=predictedweight)
